            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        <h4 class="modal-title">History Category F Level 5</h4>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="tab-content">
                            <div id="specialHistoryData" class="container tab-pane active"><br>
                                <div class="row">
                                    <!-- Modal Body -->
                                    <div id="historyContent_Level5" class="scrollspy-example"
    ...
    <div class="container">
        <footer class="d-flex justify-content-center align-items-center" style="min-height: 100px; margin-top:10px">
            @if ($CompetitioButton->status == 1)
                <button type="button" class="btn btn-primary me-2 competionHistoryLevel5 historybutton" data-bs-toggle="modal" data-bs-target="#specialHistory" id="modalOpen">History</button>
                <button type="button" class="btn btn-secondary me-2 startbutton" id="startGame">Start Game</button>
            @else
                <button type="button" class="btn btn-primary me-2 disabled competionHistoryLevel5" data-bs-toggle="modal" data-bs-target="#specialHistory" id="modalOpen">History</button>
...
$(document).ready(function() {
    ...
    let CompetitionCategoryLevel5 = @json($CompetitionCategoryLevel5); 
    let specialIndex = 0;
    let decimalDigit = 2;
    let fontSize = 40;
    let countdownTimer;
    ...
    function startGame() {
        ...
        function showNumbers(displayCount) {
            if (specialIndex < CompetitionCategoryLevel5.length && currentTime > 0) {
                let currentSettings = CompetitionCategoryLevel5[specialIndex];
                    ...
                    switch (type) {
                        case 'plus':
                            ...
                            function storeHistory(displayTextArray, totalResult, userAnswer) {
                                let storedHistory = JSON.parse(localStorage.getItem('gameHistoryLevel5')) || [];
                                storedHistory.push({
                                    numbers: displayTextArray,  // Array of random numbers
                                    sum: totalResult,  // Sum of the numbers
                                    userAnswer: userAnswer  // User's answer
                                });
                                localStorage.setItem('gameHistoryLevel5', JSON.stringify(storedHistory));
    ...
    $('#specialClearData').on('click', function() {
        localStorage.removeItem('gameHistoryLevel5');
        $('#historyContent_Level5').empty();
        historyData = [];
        alert('History cleared!');
    });
    $('.competionHistoryLevel5').on('click', function() {
    let storedHistory = JSON.parse(localStorage.getItem('gameHistoryLevel5')) || [];
    ...
    if (storedHistory.length === 0) {
        // $('#historyContent_Level5').append('<p>No history found.</p>');
    } else {
    storedHistory.forEach((entry, index) => {
        $('#historyContent_Level5').append('<p><strong>Question ' + (index + 1) + ':</strong></p>');
        
        if (Array.isArray(entry.numbers)) {
            entry.numbers.forEach(numbers => {
                $('#historyContent_Level5').append('<p> ' + numbers + '</p>');
            });
        } else {
            $('#historyContent_Level5').append('<p>Random Number data is not available.</p>');
        }
        
        $('#historyContent_Level5').append('<p><strong>Result:</strong> ' + entry.sum + '</p>');
        $('#historyContent_Level5').append('<p><strong>Your Answer:</strong> ' + entry.userAnswer + '</p><hr>');
    });
}
