...
<body>
    <div class="container-fluid mt-3">
        <p class="info-text">{{ $CompetionRegiInfo->student_fullName ?? '' }}<br>
            Group-C <br>
            @if ($CompetionResult && $CompetionResult->status == 1)
            <span class="badge badge-primary text-warning">
                <i class="fas fa-trophy"></i> Champion
            </span>
        @elseif ($CompetionResult && $CompetionResult->status == 2)
            <span class="badge badge-success text-warning">
                <i class="fas fa-medal"></i> First Runner Up
            </span>
        @elseif ($CompetionResult && $CompetionResult->status == 3)
            <span class="badge badge-info text-warning">
                <i class="fas fa-award"></i> Second Runner Up
            </span>
        @else
    ...
    <div class="container">
        <footer class="d-flex justify-content-center align-items-center" style="min-height: 100px; margin-top:10px">
            @if ($CompetitioButton->status == 1)
                <button type="button" class="btn btn-primary me-2 FlashHistory6" data-bs-toggle="modal" data-bs-target="#specialHistory" id="modalOpen">History</button>
                <button type="button" class="btn btn-secondary me-2" id="startGame">Start Game</button>
            @else
                <button type="button" class="btn btn-primary me-2 disabled FlashHistory6" data-bs-toggle="modal" data-bs-target="#specialHistory" id="modalOpen">History</button>
...
$(document).ready(function() {
        ...
        function showNumbers(displayCount) {
            ...
            if (specialIndex < FlashLevel6.length && timeLeft > 0) {
                ...
                function storeHistory(randomNumbers, loopSum, userAnswer) {
                    let storedHistory = JSON.parse(localStorage.getItem('gameHistoryFlash6')) || [];
                    storedHistory.push({
                        numbers: randomNumbers,
                        sum: loopSum,
                        userAnswer: userAnswer
                    });
                    localStorage.setItem('gameHistoryFlash6', JSON.stringify(storedHistory));
                }
    ...
    $('#startGame').on('click', function() {
        specialIndex = 0;
        $('#historyContent_flash6').empty();
        timeLeft = 60; // Reset timeLeft for a new game
        startGame();
    });
    $('#startGame').prop('disabled', false);
    $('#Flash6ClearData').on('click', function() {
        if (confirm('Are you sure you want to clear the history data?')) {
        localStorage.removeItem('gameHistoryFlash6');
        $('#historyContent_flash6').empty();
        historyData = [];
        } else {
        }
    ...
    $('.FlashHistory6').on('click', function() {
    let storedHistory = JSON.parse(localStorage.getItem('gameHistoryFlash6')) || [];

    if (storedHistory.length === 0) {
        // $('#historyContent_flash6').append('<p>No history found.</p>');
    } else {
    storedHistory.forEach((entry, index) => {
        $('#historyContent_flash6').append('<p><strong>Question ' + (index + 1) + ':</strong></p>');
        
        if (Array.isArray(entry.numbers)) {
            entry.numbers.forEach(num => {
                $('#historyContent_flash6').append('<p> ' + num + '</p>');
            });
        } else {
            $('#historyContent_flash6').append('<p>Random Number data is not available.</p>');
        }
        
        $('#historyContent_flash6').append('<p><strong>Sum:</strong> ' + entry.sum + '</p>');
        $('#historyContent_flash6').append('<p><strong>Your Answer:</strong> ' + entry.userAnswer + '</p><hr>');
    });
}
