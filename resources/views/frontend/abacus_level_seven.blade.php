            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        <h4 class="modal-title">History Category H Level 7</h4>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="tab-content">
                            <div id="specialHistoryData" class="container tab-pane active"><br>
                                <div class="row">
                                    <!-- Modal Body -->
                                    <div id="historyContent_Level7" class="scrollspy-example"
    ...
    <div class="container">
        <footer class="d-flex justify-content-center align-items-center" style="min-height: 100px; margin-top:10px">
            @if ($CompetitioButton->status == 1)
                <button type="button" class="btn btn-primary me-2 competionHistoryLevel7 historybutton" data-bs-toggle="modal" data-bs-target="#specialHistory" id="modalOpen">History</button>
                <button type="button" class="btn btn-secondary me-2 startbutton" id="startGame">Start Game</button>
            @else
                <button type="button" class="btn btn-primary me-2 disabled competionHistoryLevel7" data-bs-toggle="modal" data-bs-target="#specialHistory" id="modalOpen">History</button>
...
$(document).ready(function() {
    ...
    let CompetitionLevel7 = @json($CompetitionLevel7); 
    let specialIndex = 0;
    let decimalDigit = 2;
    let fontSize = 40;
    let countdownTimer;
    ...
    function startGame() {
        ...
        function showNumbers(displayCount) {
            if (specialIndex < CompetitionLevel7.length && currentTime > 0) {
                let currentSettings = CompetitionLevel7[specialIndex];
                    ...
                    switch (type) {
                        case 'plus':
                            ...
                            function storeHistory(displayTextArray, totalResult, userAnswer) {
                                let storedHistory = JSON.parse(localStorage.getItem('gameHistoryLevel7')) || [];
                                storedHistory.push({
                                    numbers: displayTextArray,  // Array of random numbers
                                    sum: totalResult,  // Sum of the numbers
                                    userAnswer: userAnswer  // User's answer
                                });
                                localStorage.setItem('gameHistoryLevel7', JSON.stringify(storedHistory));
    ...
    $('#specialClearData').on('click', function() {
        localStorage.removeItem('gameHistoryLevel7');
        $('#historyContent_Level7').empty();
        historyData = [];
        alert('History cleared!');
    });
    $('.competionHistoryLevel7').on('click', function() {
    let storedHistory = JSON.parse(localStorage.getItem('gameHistoryLevel7')) || [];
    ...
    if (storedHistory.length === 0) {
        // $('#historyContent_Level7').append('<p>No history found.</p>');
    } else {
    storedHistory.forEach((entry, index) => {
        $('#historyContent_Level7').append('<p><strong>Question ' + (index + 1) + ':</strong></p>');
        
        if (Array.isArray(entry.numbers)) {
            entry.numbers.forEach(numbers => {
                $('#historyContent_Level7').append('<p> ' + numbers + '</p>');
            });
        } else {
            $('#historyContent_Level7').append('<p>Random Number data is not available.</p>');
        }
        
        $('#historyContent_Level7').append('<p><strong>Result:</strong> ' + entry.sum + '</p>');
        $('#historyContent_Level7').append('<p><strong>Your Answer:</strong> ' + entry.userAnswer + '</p><hr>');
    });
}
