    ...
    <div class="container">
        <footer class="d-flex justify-content-center align-items-center" style="min-height: 100px; margin-top:10px">
            @if ($CompetitioButton->status == 1)
                <button type="button" class="btn btn-primary me-2 FlashHistory1 historybutton" data-bs-toggle="modal" data-bs-target="#specialHistory" id="modalOpen">History</button>
                <button type="button" class="btn btn-secondary me-2 startbutton" id="startGame">Start Game</button>
            @else
                <button type="button" class="btn btn-primary me-2 disabled FlashHistory1" data-bs-toggle="modal" data-bs-target="#specialHistory" id="modalOpen">History</button>
...
$(document).ready(function() {
    ...
    let FlashLevel1 = @json($FlashLevel1); 
    let specialIndex = 0;
    let decimalDigit = 2;
    let fontSize = 100;
    let countdownTimer;
    ...
    function startGame() {
        ...
        function showNumbers(displayCount) {
            ...
            if (specialIndex < FlashLevel1.length && timeLeft > 0) {
                ...
                function storeHistory(randomNumbers, loopSum, userAnswer) {
                    let storedHistory = JSON.parse(localStorage.getItem('gameHistoryFlash1')) || [];
                    storedHistory.push({
                        numbers: randomNumbers,
                        sum: loopSum,
                        userAnswer: userAnswer
                    });
                    localStorage.setItem('gameHistoryFlash1', JSON.stringify(storedHistory));
                }
    ...
    $('#startGame').on('click', function() {
        specialIndex = 0;
        $('#historyContent_flash1').empty();
        timeLeft = 60; // Reset timeLeft for a new game
        startGame();
    });
    $('#startGame').prop('disabled', false);
    $('#Flash1ClearData').on('click', function() {
        if (confirm('Are you sure you want to clear the history data?')) {
        localStorage.removeItem('gameHistoryFlash1');
        $('#historyContent_flash1').empty();
        historyData = [];
        } else {
        }
    ...
    $('.FlashHistory1').on('click', function() {
    let storedHistory = JSON.parse(localStorage.getItem('gameHistoryFlash1')) || [];

    if (storedHistory.length === 0) {
        // $('#historyContent_flash1').append('<p>No history found.</p>');
    } else {
    storedHistory.forEach((entry, index) => {
        $('#historyContent_flash1').append('<p><strong>Question ' + (index + 1) + ':</strong></p>');
        
        if (Array.isArray(entry.numbers)) {
            entry.numbers.forEach(num => {
                $('#historyContent_flash1').append('<p> ' + num + '</p>');
            });
        } else {
            $('#historyContent_flash1').append('<p>Random Number data is not available.</p>');
        }
        
        $('#historyContent_flash1').append('<p><strong>Sum:</strong> ' + entry.sum + '</p>');
        $('#historyContent_flash1').append('<p><strong>Your Answer:</strong> ' + entry.userAnswer + '</p><hr>');
    });
}
