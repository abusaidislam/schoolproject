        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h4 class="modal-title">History Category B2 - Level 01</h4>
                ...
                <div class="modal-body">
                    ...
                    <div class="tab-content">
                        <div id="specialHistoryData" class="container tab-pane active"><br>
                            <div class="row">
                                <!-- Modal Body -->
                                <div id="historyContent_b2_level1" class="scrollspy-example"
...
let CategoryB2Level1 = @json($CategoryB2Level1); 
let specialIndex = 0;
let decimalDigit = 2;
let fontSize = 40;
let countdownTimer;
...
function startGame() {
    ...
    function showNumbers(displayCount) {
        if (specialIndex < CategoryB2Level1.length && currentTime > 0) {
            let currentSettings = CategoryB2Level1[specialIndex];
            ...
            setTimeout(() => {
                ...
                for (let i = 0; i < nRow; i++) {
                    ...
                    // $('#historyContent_b2_level1').append('<p> random ' + displayText + '</p>');
                }

                // $('#historyContent_b2_level1').append('<p>sum = : ' + loopSum + '</p><hr>');
                function storeHistory(randomNumbers, loopSum, userAnswer) {
                    let storedHistory = JSON.parse(localStorage.getItem('gameHistoryB2Level1')) || [];
                    storedHistory.push({
                        numbers: randomNumbers,  // Array of random numbers
                        sum: loopSum,  // Sum of the numbers
                        userAnswer: userAnswer  // User's answer
                    });
                    localStorage.setItem('gameHistoryB2Level1', JSON.stringify(storedHistory));
...
$('#startGame').on('click', function() {
    specialIndex = 0;
    $('#historyContent_b2_level1').empty();
    currentTime = gameDuration;
    startGame();
});

$('#specialClearData').on('click', function() {
    localStorage.removeItem('gameHistoryB2Level1');
    $('#historyContent_b2_level1').empty();
    historyData = [];
...
if (storedHistory.length === 0) {
    $('#historyContent_b2_level1').append('<p>No history found.</p>');
} else {
storedHistory.forEach((entry, index) => {
    $('#historyContent_b2_level1').append('<p><strong>Question ' + (index + 1) + ':</strong></p>');
    
    if (Array.isArray(entry.numbers)) {
        entry.numbers.forEach(num => {
            $('#historyContent_b2_level1').append('<p> ' + num + '</p>');
        });
    } else {
        $('#historyContent_b2_level1').append('<p>Random Number data is not available.</p>');
    }
    
    $('#historyContent_b2_level1').append('<p><strong>Sum:</strong> ' + entry.sum + '</p>');
    $('#historyContent_b2_level1').append('<p><strong>Your Answer:</strong> ' + entry.userAnswer + '</p><hr>');
});
}
