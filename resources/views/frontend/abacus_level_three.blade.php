...
@section('content')
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        <h4 class="modal-title">History Category D Level 3</h4>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="tab-content">
                            <div id="specialHistoryData" class="container tab-pane active"><br>
                                <div class="row">
                                    <!-- Modal Body -->
                                    <div id="historyContent_Level3" class="scrollspy-example"
    ...
    <div class="container">
        <footer class="d-flex justify-content-center align-items-center" style="min-height: 100px; margin-top:10px">
            ...
            <button type="button" class="btn btn-primary me-2 competionHistoryLevel3  historybutton" data-bs-toggle="modal" data-bs-target="#specialHistory" id="modalOpen">History</button>
            <button type="button" class="btn btn-secondary me-2 startbutton" id="startGame">Start Game</button>
        @else
            <button type="button" class="btn btn-primary me-2 disabled competionHistoryLevel3" data-bs-toggle="modal" data-bs-target="#specialHistory" id="modalOpen">History</button>
...
$(document).ready(function() {
    ...
    let CompetitionCategoryLevel3 = @json($CompetitionCategoryLevel3); // Make sure this is outputting a valid JSON array
    let specialIndex = 0;
    let decimalDigit = 2;
    let fontSize = 40;
    let countdownTimer;
    ...
    function startGame() {
        ...
        function showNumbers(displayCount) {
            if (specialIndex < CompetitionCategoryLevel3.length && currentTime > 0) {
                let currentSettings = CompetitionCategoryLevel3[specialIndex];
                let dLength = currentSettings.num_length;
                let nDisplay = currentSettings.num_display;
                let nRow = currentSettings.num_row;
                let conditionNum = currentSettings.num_condition; // Ensure conditionNum is accessible here
                ...
                setTimeout(() => {
                    ...
                    for (let i = 0; i < nRow; i++) {
                        ...
                        // $('#historyContent_Level3').append('<p> random ' + displayText + '</p>');
                    }

                     // $('#historyContent_Level3').append('<p>sum = : ' + loopSum + '</p><hr>');
                     function storeHistory(randomNumbers, loopSum, userAnswer) {
                        let storedHistory = JSON.parse(localStorage.getItem('gameHistoryLevel3')) || [];
                        storedHistory.push({
                            numbers: randomNumbers,  // Array of random numbers
                            sum: loopSum,  // Sum of the numbers
                            userAnswer: userAnswer  // User's answer
                        });
                        localStorage.setItem('gameHistoryLevel3', JSON.stringify(storedHistory));
                    ...
                    // Your existing keypress event handler
                    ...
                    $('#answerInput').off('keypress').on('keypress', function(e) {
                                if (e.which === 13) {
                                    ...
                                    // $('#historyContent_Level3').append('<p>user data =: ' + userAnswer + '</p><hr>');
        ...
        function endGame() {
            ...
            $('#startGame').prop('disabled', false);//just one time game run button comment//
    ...
    $('#startGame').on('click', function() {
        specialIndex = 0;
        // $('#historyContent_Level3').empty();
        currentTime = gameDuration; // Reset the timer
        startGame();
    });
    $('#specialClearData').on('click', function() {
        localStorage.removeItem('gameHistoryLevel3');
        $('#historyContent_Level3').empty();
        historyData = [];
        alert('History cleared!');
    });

    $('.competionHistoryLevel3').on('click', function() {
    let storedHistory = JSON.parse(localStorage.getItem('gameHistoryLevel3')) || [];
    ...
    if (storedHistory.length === 0) {
        // $('#historyContent_Level3').append('<p>No history found.</p>');
    } else {
    storedHistory.forEach((entry, index) => {
        $('#historyContent_Level3').append('<p><strong>Question ' + (index + 1) + ':</strong></p>');
        
        if (Array.isArray(entry.numbers)) {
            entry.numbers.forEach(num => {
                $('#historyContent_Level3').append('<p> ' + num + '</p>');
            });
        } else {
            $('#historyContent_Level3').append('<p>Random Number data is not available.</p>');
        }
        
        $('#historyContent_Level3').append('<p><strong>Sum:</strong> ' + entry.sum + '</p>');
        $('#historyContent_Level3').append('<p><strong>Your Answer:</strong> ' + entry.userAnswer + '</p><hr>');
    });
}
