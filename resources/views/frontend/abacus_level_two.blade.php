...
@section('content')
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        <h4 class="modal-title">History Category C Level 2</h4>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="tab-content">
                            <div id="specialHistoryData" class="container tab-pane active"><br>
                                <div class="row">
                                    <!-- Modal Body -->
                                    <div id="historyContent_Level2" class="scrollspy-example"
    ...
    <div class="container">
        <footer class="d-flex justify-content-center align-items-center" style="min-height: 100px; margin-top:10px">
            ...
            <button type="button" class="btn btn-primary me-2 competionHistoryLevel2  historybutton" data-bs-toggle="modal" data-bs-target="#specialHistory" id="modalOpen">History</button>
            <button type="button" class="btn btn-secondary me-2 startbutton" id="startGame">Start Game</button>
        @else
            <button type="button" class="btn btn-primary me-2 disabled competionHistoryLevel2" data-bs-toggle="modal" data-bs-target="#specialHistory" id="modalOpen">History</button>
...
$(document).ready(function() {
    ...
    let CompetitionCategoryLevel2 = @json($CompetitionCategoryLevel2); // Make sure this is outputting a valid JSON array
    let specialIndex = 0;
    let decimalDigit = 2;
    let fontSize = 40;
    let countdownTimer;
    ...
    function startGame() {
        ...
        function showNumbers(displayCount) {
            if (specialIndex < CompetitionCategoryLevel2.length && currentTime > 0) {
                let currentSettings = CompetitionCategoryLevel2[specialIndex];
                ...
                setTimeout(() => {
                    ...
                    for (let i = 0; i < nRow; i++) {
                        ...
                        // $('#historyContent_Level2').append('<p> random ' + displayText + '</p>');
                    }

                     // $('#historyContent_Level2').append('<p>sum = : ' + loopSum + '</p><hr>');
                     function storeHistory(randomNumbers, loopSum, userAnswer) {
                        let storedHistory = JSON.parse(localStorage.getItem('gameHistoryLevel2')) || [];
                        storedHistory.push({
                            numbers: randomNumbers,  // Array of random numbers
                            sum: loopSum,  // Sum of the numbers
                            userAnswer: userAnswer  // User's answer
                        });
                        localStorage.setItem('gameHistoryLevel2', JSON.stringify(storedHistory));
                    ...
                    $('#answerInput').off('keypress').on('keypress', function(e) {
                                if (e.which === 13) {
                                    ...
                                    // $('#historyContent_Level2').append('<p>user data =: ' + userAnswer + '</p><hr>');
    ...
    $('#startGame').on('click', function() {
        specialIndex = 0;
        // $('#historyContent_Level2').empty();
        currentTime = gameDuration; // Reset the timer
        startGame();
    });
    $('#specialClearData').on('click', function() {
        localStorage.removeItem('gameHistoryLevel2');
        $('#historyContent_Level2').empty();
        historyData = [];
        alert('History cleared!');
    });

    $('.competionHistoryLevel2').on('click', function() {
    let storedHistory = JSON.parse(localStorage.getItem('gameHistoryLevel2')) || [];
    ...
    if (storedHistory.length === 0) {
        // $('#historyContent_Level2').append('<p>No history found.</p>');
    } else {
    storedHistory.forEach((entry, index) => {
        $('#historyContent_Level2').append('<p><strong>Question ' + (index + 1) + ':</strong></p>');
        
        if (Array.isArray(entry.numbers)) {
            entry.numbers.forEach(num => {
                $('#historyContent_Level2').append('<p> ' + num + '</p>');
            });
        } else {
            $('#historyContent_Level2').append('<p>Random Number data is not available.</p>');
        }
        
        $('#historyContent_Level2').append('<p><strong>Sum:</strong> ' + entry.sum + '</p>');
        $('#historyContent_Level2').append('<p><strong>Your Answer:</strong> ' + entry.userAnswer + '</p><hr>');
    });
}
