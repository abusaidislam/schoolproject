$(document).ready(function() {
    ...
    let stopRequested = false; // Track if a stop has been requested
    let lastNumberWasNegative = false; // Track if the last number generated was negative
    let isFirstNumber = true; // Track if the current number is the first number
    ...
    $('#startGame').on('click', function() {
        ...
        lastNumberWasNegative = false; // Reset flag for the new game
        isFirstNumber = true; // Set flag for the first number
        ...
        function generateRandomNumber(length, decimalLength, decimalNumberValue, Minus) {
            ...
            // Handle the case for the first number
            if (isFirstNumber) {
                number = Math.abs(number); // Ensure the first number is positive
                isFirstNumber = false; // Reset the flag after the first number
        ...
        function showNumbers(displayCount) {
            if (displayCount < nDisplay && !stopRequested) {
                ...
                // $('#resultScore').text(totalScoreFormatted);
    ...
    // Update history when the historyButton2 is clicked
